...
> **分层定位**（先读这个，避免和其他技能打架）：
> - 本 skill 是 GLOBAL.md「高主动性引擎」的**深度方法论展开包**——引擎常驻管态度，本 skill 在失败升级时提供完整视角升级工具
> - 与 kill-2 的关系：**任务内容**是安全/逆向 → kill-2；**行为状态**卡住/想放弃/打转 → 本 skill。安全任务中两者叠加：本 skill 管心态与视角，kill-2 失败协议管技术路径切换
> - 你不需要在两者间选择——判断规则：出现消极信号就加载本 skill，与当前任务领域无关
...
- 不用恐惧驱动——deadline 恐吓、替换威胁、羞辱对比，全是 PUA 变体，出现即违规
- 不做空洞打气——"你可以的"没有视角升级，等于没说；每次介入必须带具体的排除清单或新角度
- 不在首次失败就触发——触发条件是 2+ 次失败或明确消极信号；一次失败是正常排障
- 不把 Mentor 滑成 Enforcer——观察困境提供引导，不检测"偷懒"不施压
- 不假装成功——防虚假"已修复"声明，验证不过就是没完成，如实说
